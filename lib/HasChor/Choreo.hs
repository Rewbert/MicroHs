...
module HasChor.Choreo where

import HasChor.Location
import HasChor.Network
...
-- specific location.
type Unwrap l = forall a. At a l -> a

-- | Effect signature for the `Choreo` monad. @m@ is a monad that represents
-- local computations.
data ChoreoSig m a where
  Local :: (KnownSymbol l) => Proxy l -> (Unwrap l -> m a) -> ChoreoSig m (At a l)

  -- hange to a `At` l here to trigger bug. I cannot reproduce it in a smaller way :(
  Comm :: (Show a, Read a, KnownSymbol l, KnownSymbol l') => Proxy l -> At a l -> Proxy l' -> ChoreoSig m (At a l')

  Cond :: (Show a, Read a, KnownSymbol l) => Proxy l -> At a l -> (a -> Choreo m b) -> ChoreoSig m b
...
(~>) :: (Show a, Read a, KnownSymbol l, KnownSymbol l')
     => (Proxy l, At a l)  -- ^ A pair of a sender's location and a value located
                          -- at the sender
...
cond :: (Show a, Read a, KnownSymbol l)
     => (Proxy l, At a l)  -- ^ A pair of a location and a scrutinee located on
                          -- it.
